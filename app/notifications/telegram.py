...
class TelegramProvider(NotificationProvider):
    ...
    def is_configured(self) -> bool:
        """Checks whether bot token and chat ID are configured."""
        return bool(self.bot_token and self.chat_id)

    async def send_message(self, text: str) -> bool:
        """Sends raw text message to configured chat ID."""
        if not self.is_configured():
            logger.warning("Telegram não configurado (TELEGRAM_BOT_TOKEN ou TELEGRAM_CHAT_ID ausente). Mensagem não enviada.")
            return False
        ...
        try:
            ...
            logger.error(f"Erro ao enviar mensagem pelo Telegram: {err}")
            return False
