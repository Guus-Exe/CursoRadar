...
def normalize_shift(raw_shift: Optional[str], hours_text: Optional[str] = None) -> str:
    ...
    if raw_shift:
        ...
        if clean in SHIFT_MAP:
            return SHIFT_MAP[clean]
        for key, val in SHIFT_MAP.items():
            if key in clean:
                return val

    # Fallback to hours inspection
    if hours_text:
        h_clean = hours_text.lower()
        if any(w in h_clean for w in ["noturno", "noite"]):
            return "Noturno"
        if any(w in h_clean for w in ["tarde", "vespertino"]):
            return "Tarde"
        if any(w in h_clean for w in ["manhã", "manha", "matutino"]):
            return "Manhã"

        # Check starting hour: e.g. "14h às 18h", "19:00 às 22:30", "08:00"
...
class OfferParser:
    ...
    def parse_html_offer(
        ...
        horarios: List[str] = []
        turno = "Noturno"
        for text_elem in soup.find_all(["p", "span", "div"], string=re.compile(r"(?:noturno|manhã|tarde|\d{2}h\s*às\s*\d{2}h)", re.I)):
            s = text_elem.get_text(strip=True)
            if len(s) < 100:
                horarios.append(s)
                turno = normalize_shift(None, s)
