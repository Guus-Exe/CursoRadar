...
class WorkerHealthModel(Base):
    """Table to record worker heartbeats and operational metrics."""

    __tablename__ = "worker_health"

    id = Column(Integer, primary_key=True, autoincrement=True)
    worker_id = Column(String(100), nullable=False)
    heartbeat_at = Column(DateTime, default=datetime.now, nullable=False)
    last_cycle_at = Column(DateTime, nullable=True)
    cycle_duration_seconds = Column(Integer, default=0)
    offers_checked = Column(Integer, default=0)
    offers_succeeded = Column(Integer, default=0)
    offers_failed = Column(Integer, default=0)
    status = Column(String(50), default="healthy", nullable=False)
    metadata_json = Column(Text, nullable=True)


class SystemErrorModel(Base):
    """Table to record errors and exceptions for observability."""
...
class Database:
    ...
    def record_worker_heartbeat(
        ...
        """Records a heartbeat with performance and execution metrics."""
        with self.get_session() as session:
            health = WorkerHealthModel(
                worker_id=worker_id,
                heartbeat_at=datetime.now(),
                last_cycle_at=last_cycle_at or datetime.now(),
                cycle_duration_seconds=int(duration_seconds),
    ...
    def record_system_error(
        ...
        """Logs an operational or scraping error for administrative inspection."""
        with self.get_session() as session:
    ...
    def get_latest_worker_health(self, worker_id: Optional[str] = None) -> Optional[WorkerHealthModel]:
        """Retrieves most recent health record."""
        with self.get_session() as session:
            stmt = select(WorkerHealthModel).order_by(WorkerHealthModel.heartbeat_at.desc())
            if worker_id:
                stmt = stmt.where(WorkerHealthModel.worker_id == worker_id)
